...
Rule 2: Missing 'Operating_Margin' column -> mark status as 'delete'.
    ...
    for sector_name, sector_info in config.get('sectors', {}).items():
        for company in sector_info.get('companies', []):
            ...
            if not csv_path.exists():
                ...
                company['status'], company['reason'] = 'delete', 'Rule 1: CSV file not found'
            ...
            try:
                ...
                if 'Operating_Margin' not in df.columns:
                    logger.warning(f"  ✗ {ticker}: Missing Operating_Margin.")
                    csv_path.unlink()
                    company['status'], company['reason'] = 'delete', 'Rule 2: Missing Operating_Margin column'
                    stats["deleted_anomaly"] += 1
                    continue
                
                valid_margins = df['Operating_Margin'].dropna()
                valid_count = len(valid_margins)

                # Rule 3
                if valid_count < 4:
                    logger.warning(f"  ✗ {ticker}: Only {valid_count} quarters.")
                    csv_path.unlink()
                    company['status'], company['reason'] = 'delete', f'Rule 3: Insufficient data ({valid_count} quarters)'
                ...
                if mean_margin >= 0.95:
                    ...
                    csv_path.unlink()
                    company['status'], company['reason'] = 'delete', f'Rule 4A: Anomalous mean margin ({mean_margin:.2f})'
                    stats["deleted_anomaly"] += 1
                    continue
                ...
                if std_margin == 0:
                    ...
                    csv_path.unlink()
                    company['status'], company['reason'] = 'delete', 'Rule 4B: Standard deviation is zero (Fake/Flat data)'
                    stats["deleted_anomaly"] += 1
                    continue
                ...
                company['status'], company['reason'] = 'delete', f'Error: {str(e)}'
                stats["deleted_anomaly"] += 1
